...
export const NavItem:FC<Props> = ({refValue, setMenuIsActive, menuIsActive}):JSX.Element => {
  ...
  return (
    <div 
        className={`nav-item ${menuIsActive ? 'nav-item-active' : ''}`}
        style={{
            maxHeight: (menuIsActive ? '580px' : '0px')
        }}    
    >
        <div className='nav-item_container'>
            <div className='nav-item_container-options'>
                <div className='nav-item_options' onClick={handleHostingClick}>
                    Hosting
                    <svg className={`expand ${hostingIsActive ? 'expand-transform' : ''}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path d="M480-345 240-585l43-43 197 198 197-197 43 43-240 239Z"/></svg>
                </div>
                
                <div className={`nav-item_options-items_container ${hostingIsActive ? 'nav-item-active' : ''}`}>
